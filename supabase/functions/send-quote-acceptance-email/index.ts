...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    // Fetch quote details with related lead/client info
    ...
    if (quote.lead_id) {
      const { data: lead } = await supabase
        .from("leads")
        .select("id, lead_name, client_id")
        .eq("id", quote.lead_id)
        .single();
      leadData = lead;
      
      // Get client from lead if no direct client_id
      if (lead?.client_id) {
    ...
    // Get direct client if specified
    ...
    const clientEmailHtml = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <div style="padding: 32px; background-color: #ffffff;">
          ...
          <p style="color: #374151; font-size: 16px;">
            Thank you for accepting our proposal for <strong>${leadName}</strong>. 
          ...
          <p style="color: #374151; font-size: 16px;">
            Our team will be in touch shortly to discuss the next steps and finalise any remaining details.
          </p>
          <p style="color: #374151; font-size: 16px;">
    ...
    // 2. Send internal notification email to EventPix
      ...
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <div style="padding: 32px; background-color: #ffffff;">
          <p style="color: #374151; font-size: 16px; margin-top: 0;">
            Great news! A client has accepted a proposal.
          </p>
          <div style="background-color: #f3f4f6; padding: 20px; border-radius: 8px; margin: 24px 0;">
            <table style="width: 100%; border-collapse: collapse;">
              <tr>
                <td style="padding: 8px 0; color: #6b7280;">Client</td>
                <td style="padding: 8px 0; color: #111827; font-weight: bold; text-align: right;">${clientName}</td>
              </tr>
              <tr>
                <td style="padding: 8px 0; color: #6b7280;">Job</td>
                <td style="padding: 8px 0; color: #111827; font-weight: bold; text-align: right;">${leadName}</td>
              </tr>
              <tr>
                <td style="padding: 8px 0; color: #6b7280;">Quote</td>
                <td style="padding: 8px 0; color: #111827; font-weight: bold; text-align: right;">${quoteNumber}</td>
              </tr>
              <tr>
                <td style="padding: 8px 0; color: #6b7280;">Value</td>
                <td style="padding: 8px 0; color: #059669; font-weight: bold; font-size: 18px; text-align: right;">${totalFormatted}</td>
              </tr>
            </table>
          </div>
          ...
          <p style="color: #374151; font-size: 14px; margin-top: 24px;">
            Please follow up with the client to confirm next steps and convert to a job if not already done.
          </p>
        </div>
      </div>
    `;
    ...
    const sendEmail = async (to: string, subject: string, html: string) => {
      ...
      for (let attempt = 0; attempt < maxRetries; attempt++) {
        ...
        if (!response.ok) {
          lastError = result.message || "Failed to send";
          console.error(`Email to ${to} failed:`, result);
          continue;
        }
    ...
    const clientResult = await sendEmail(
      acceptedByEmail,
      `Thank you for your booking confirmation - ${leadName}`,
      clientEmailHtml
    );

    if (clientResult.success) {
      console.log(`Client confirmation email sent to: ${acceptedByEmail}`);
      ...
      await supabase.from("email_logs").insert({
        ...
        body_preview: `Thank you for accepting our proposal for ${leadName}. We're excited to work with you!`,
        status: "sent",
    ...
    const internalResult = await sendEmail(
      "pix@eventpix.com.au",
      `🎉 Quote Accepted: ${clientName} - ${leadName} (${totalFormatted})`,
      internalEmailHtml
    );

    if (internalResult.success) {
      console.log("Internal notification email sent to: pix@eventpix.com.au");
