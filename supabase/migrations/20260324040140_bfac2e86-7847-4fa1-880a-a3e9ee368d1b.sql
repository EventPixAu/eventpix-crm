...
BEGIN
  ...
  IF COALESCE((p_input #>> '{options,create_admin_setup_tasks}')::boolean, true) THEN
    ...
    VALUES
      ('event', v_event_id, 'admin_setup', 'Review event brief', CASE WHEN v_event_date IS NOT NULL THEN (v_event_date::timestamp - INTERVAL '7 days') ELSE NULL END, v_user_id, 'pending', 'medium', v_user_id),
      ('event', v_event_id, 'admin_setup', 'Confirm crew assignments', CASE WHEN v_event_date IS NOT NULL THEN (v_event_date::timestamp - INTERVAL '3 days') ELSE NULL END, v_user_id, 'pending', 'high', v_user_id),
      ('event', v_event_id, 'admin_setup', 'Send pre-event client confirmation', CASE WHEN v_event_date IS NOT NULL THEN (v_event_date::timestamp - INTERVAL '1 day') ELSE NULL END, v_user_id, 'pending', 'medium', v_user_id);

    GET DIAGNOSTICS v_task_count = ROW_COUNT;
  END IF;

  UPDATE public.quotes SET event_id = v_event_id WHERE lead_id = v_lead_id;
  GET DIAGNOSTICS v_quote_count = ROW_COUNT;

  UPDATE public.contracts SET event_id = v_event_id WHERE lead_id = v_lead_id;
  ...
  IF v_lead_record.client_id IS NOT NULL THEN
    ...
    SET status = 'active_event'
    WHERE id = v_lead_record.client_id;
  END IF;

  INSERT INTO public.audit_log (action, event_id, actor_user_id, before, after)
  VALUES (
...
AS $function$
BEGIN
  RETURN public.convert_enquiry_to_event(
    jsonb_build_object(
      'enquiry_id', p_lead_id,
