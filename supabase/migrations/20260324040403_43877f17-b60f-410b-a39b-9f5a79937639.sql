...
BEGIN
  ...
  IF COALESCE((p_input #>> '{options,create_admin_setup_tasks}')::boolean, true) THEN
    ...
    VALUES
      ...
      ('event', v_event_id, 'prep', 'Confirm crew assignments', CASE WHEN v_event_date IS NOT NULL THEN (v_event_date::timestamp - INTERVAL '3 days') ELSE NULL END, v_user_id, 'open', 'high', v_user_id),
      ('event', v_event_id, 'email', 'Send pre-event client confirmation', CASE WHEN v_event_date IS NOT NULL THEN (v_event_date::timestamp - INTERVAL '1 day') ELSE NULL END, v_user_id, 'open', 'normal', v_user_id);
