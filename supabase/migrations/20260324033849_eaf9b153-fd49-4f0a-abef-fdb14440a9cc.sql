...
BEGIN
  ...
  -- 8) Copy enquiry contacts with role mapping
  ...
  UPDATE event_sessions SET event_id = v_event_id, updated_at = NOW() WHERE lead_id = v_lead_id;
  GET DIAGNOSTICS v_session_count = ROW_COUNT;

  -- 9) Auto-initialize workflows
  ...
  UPDATE leads SET status = 'won', updated_at = NOW() WHERE id = v_lead_id;
