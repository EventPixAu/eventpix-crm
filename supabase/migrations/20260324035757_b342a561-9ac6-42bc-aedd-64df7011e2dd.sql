...
BEGIN
  ...
  ELSIF (p_input #>> '{venue,create,name}') IS NOT NULL AND (p_input #>> '{venue,create,name}') <> '' THEN
    INSERT INTO venues(name, address_line_1, suburb, state, postcode, country, parking_notes, access_notes)
    VALUES (
  ...
  SELECT
    ...
    CASE WHEN v_venue_id IS NOT NULL THEN (SELECT ve.address_line_1 FROM venues ve WHERE ve.id = v_venue_id) ELSE NULL END,
  ...
  IF COALESCE((p_input #>> '{options,copy_enquiry_contacts}')::boolean, true) THEN
    ...
    SELECT v_event_id, ec.contact_id,
      ...
      cc.contact_name, cc.email, COALESCE(cc.phone_mobile, cc.phone)
  ...
  -- Update lead + link quotes/contracts
  UPDATE leads SET status = 'won', updated_at = NOW(), converted_job_id = v_event_id WHERE id = v_lead_id;
  UPDATE quotes SET event_id = v_event_id WHERE lead_id = v_lead_id;
  UPDATE contracts SET event_id = v_event_id WHERE lead_id = v_lead_id;
  UPDATE email_logs SET event_id = v_event_id WHERE lead_id = v_lead_id AND event_id IS NULL;

  INSERT INTO audit_log(action, event_id, actor_user_id, before, after)
  VALUES ('event_created', v_event_id, v_user_id,
  ...
  RETURN jsonb_build_object(
    'success', true, 'event_id', v_event_id, 'venue_id', v_venue_id,
