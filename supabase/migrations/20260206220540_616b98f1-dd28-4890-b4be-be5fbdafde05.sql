...
-- Fix the convert_enquiry_to_event function to properly transfer sessions
-- Must set lead_id to NULL when transferring to event due to check constraint
...
BEGIN
  ...
  -- IMPORTANT: Must set lead_id to NULL due to check constraint (either lead_id OR event_id, not both)
  ...
  -- 9) Link ALL email logs from the lead to the new event
  UPDATE email_logs
  SET 
    event_id = v_event_id,
    updated_at = NOW()
  ...
  -- 10) Mark the lead as won and link to event
  UPDATE leads
  SET 
    status = 'won',
  ...
  -- 11) Initialize operations workflow for the event
  PERFORM public.initialize_event_workflow(v_event_id);

  -- 12) Update company status to 'active_event' if applicable
