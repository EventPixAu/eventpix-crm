...
BEGIN
  ...
  -- Determine if admin or editor defaults exist for this event type
  SELECT EXISTS (
    SELECT 1
    ...
    UNION ALL
    SELECT 1
    FROM public.editor_event_type_step_defaults d
    JOIN public.editor_workflow_master_steps s ON s.id = d.master_step_id
    WHERE d.event_type_id = p_event_type_id
      AND s.is_active = true
  ) INTO v_has_defaults;
  ...
  LOOP
    ...
    -- Remove non-completed steps that are NOT in the combined Admin + Editor defaults set (by label match)
    WITH target_labels AS (
      ...
      UNION
      SELECT s.label
      FROM public.editor_workflow_master_steps s
      WHERE s.is_active = true
        AND EXISTS (
          SELECT 1
          FROM public.editor_event_type_step_defaults d
    ...
    -- Insert any missing steps from the combined Admin + Editor defaults set
    ...
    FOR v_step IN
      ...
      WHERE NOT EXISTS (
        ...
        WHERE ews.event_id = v_event.id
          AND ews.step_label = target_steps.label
      )
      ORDER BY
        CASE phase
