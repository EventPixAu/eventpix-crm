-- Add automatic company status update when lead converts to event
...
BEGIN
  ...
  JOIN workflow_template_items wti ON wti.id = wt.id
  ...
  -- 14) AUTO-UPDATE CLIENT STATUS TO "Active Event"
  -- Only update if client_id exists and status is not already manually overridden to something specific
  IF v_lead_record.client_id IS NOT NULL THEN
    UPDATE clients
    SET 
      manual_status = 'Active Event',
      updated_at = NOW()
    WHERE id = v_lead_record.client_id
      -- Only update if current status suggests they're not already marked as Active Event
      AND (manual_status IS NULL OR manual_status NOT IN ('Active Event', 'Supplier'));
      
    -- Log the status change in audit
    INSERT INTO company_status_audit (company_id, action, old_status, new_status, changed_by)
    SELECT 
      v_lead_record.client_id,
      'auto_lead_conversion',
      c.manual_status,
      'Active Event',
      auth.uid()
    FROM clients c
    WHERE c.id = v_lead_record.client_id;
  END IF;
