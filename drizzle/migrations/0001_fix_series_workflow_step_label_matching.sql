...
BEGIN
  ...
  WITH target AS (
    SELECT DISTINCT btrim(s.label) AS label
    ...
    WHERE s.is_active = true
      ...
      AND (
        EXISTS (
          ...
          JOIN public.events e ON COALESCE(e.workflow_event_type_id, e.event_type_id) = d.event_type_id
  ...
  deleted AS (
    ...
    WHERE sw.series_id = p_series_id
      ...
      AND btrim(sw.step_label) NOT IN (SELECT label FROM target)
    RETURNING 1
  )
  ...
  FOR v_step IN
    SELECT DISTINCT ON (btrim(s.label)) s.*, btrim(s.label) AS clean_label
    FROM public.workflow_master_steps s
    WHERE s.is_active = true
      AND s.is_series_level = true
      AND (
      ...
      AND NOT EXISTS (
        ...
        WHERE sw.series_id = p_series_id AND btrim(sw.step_label) = btrim(s.label)
      )
    ORDER BY btrim(s.label), s.sort_order
  LOOP
    v_order := v_order + 1;
    INSERT INTO public.series_workflow_steps (
      ...
      p_series_id, v_step.id, v_step.clean_label, v_step.phase::workflow_phase, v_order,
  ...
  WHERE ews.event_id = e.id
    ...
    AND btrim(ews.step_label) IN (
      SELECT btrim(label) FROM public.workflow_master_steps
