CREATE UNIQUE INDEX event_contacts_unique_linked_contact_per_event
ON public.event_contacts (event_id, client_contact_id)
  ...
  IF _primary IS NOT NULL THEN
    ...
    WHERE e.event_series_id = _series_id AND c.id = _primary
      AND NOT EXISTS (
        ...
        WHERE ec1.event_id = e.id AND ec1.client_contact_id = c.id
      )
    ON CONFLICT (event_id, client_contact_id) WHERE client_contact_id IS NOT NULL DO NOTHING;
  END IF;
  ...
  IF _onsite IS NOT NULL THEN
    ...
    WHERE e.event_series_id = _series_id AND c.id = _onsite
      AND NOT EXISTS (
        ...
        WHERE ec2.event_id = e.id AND ec2.client_contact_id = c.id
      )
    ON CONFLICT (event_id, client_contact_id) WHERE client_contact_id IS NOT NULL DO NOTHING;
  END IF;
