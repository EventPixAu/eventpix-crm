CREATE OR REPLACE FUNCTION public.venue_name_key(p text)
RETURNS text LANGUAGE sql IMMUTABLE SET search_path = public AS $f$
  SELECT regexp_replace(regexp_replace(lower(coalesce(p,'')), '\m(the|pty|ltd|hotel|centre|center)\M', ' ', 'g'), '[^a-z0-9]', '', 'g')
$f$;
...
BEGIN
  ...
  IF (p_input #>> '{venue,venue_id}') IS NOT NULL AND (p_input #>> '{venue,venue_id}') <> '' THEN
    ...
    END IF;
  ELSIF (p_input #>> '{venue,create,name}') IS NOT NULL AND (p_input #>> '{venue,create,name}') <> ''
        AND EXISTS (SELECT 1 FROM public.venues v WHERE v.is_active AND public.venue_name_key(v.name) = public.venue_name_key(p_input #>> '{venue,create,name}')) THEN
    SELECT v.id INTO v_venue_id FROM public.venues v
      WHERE v.is_active AND public.venue_name_key(v.name) = public.venue_name_key(p_input #>> '{venue,create,name}')
      ORDER BY v.is_confirmed DESC NULLS LAST, (v.address_line_1 IS NOT NULL) DESC, v.created_at ASC LIMIT 1;
  ...
  IF COALESCE((p_input #>> '{options,create_worksheets}')::boolean, true) THEN
    INSERT INTO public.worksheets (event_id, template_name, phase, status)
    VALUES
      (v_event_id, 'Run Sheet', 'pre_event', 'not_started'),
      (v_event_id, 'Shot List', 'pre_event', 'not_started');
