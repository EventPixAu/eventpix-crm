  ...
  const { data: clientContacts = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data: assocs } = await supabase
        ...
        .select('contact_id, is_primary, created_at')
        .eq('company_id', seriesClientId)
        .order('created_at', { ascending: true });
      const contactIds = (assocs || []).map((a: any) => a.contact_id).filter(Boolean);
                ...
                <CardContent className="space-y-5">
                  {([
                    ...
                    },
                    {
                      ...
                      field: 'onsite_contact_id',
                      empty: 'No on-site contact set for this series.',
                    },
                  ]).map((row) => (
                    <div key={row.key} className="space-y-2">
                      ...
                      {row.contact ? (
                        <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-sm">
                          <div>
                            <div className="font-medium">{contactLabel(row.contact)}</div>
                            {row.contact.role_title && (
                              <div className="text-xs text-muted-foreground">{row.contact.role_title}</div>
                            )}
                          </div>
                          {row.contact.email && (
                            <a href={`mailto:${row.contact.email}`} className="text-primary hover:underline">
                              {row.contact.email}
                            </a>
                          )}
                          {contactPhone(row.contact) && (
                            <a href={`tel:${contactPhone(row.contact)}`} className="text-primary hover:underline">
                              {contactPhone(row.contact)}
                            </a>
                          )}
                        </div>
