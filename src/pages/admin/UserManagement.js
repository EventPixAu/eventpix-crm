...
import { UserPlus, Mail, MoreHorizontal, RefreshCw, XCircle, Check, Clock, AlertCircle, AlertTriangle, CheckCircle } from 'lucide-react';
import { formatDistanceToNow } from 'date-fns';
import { toast } from 'sonner';
import { supabase } from '@/lib/supabase';
import { useUsers, useInvitations, useInviteUser, useResendInvitation, useRevokeInvitation, useSetUserActive, useSetUserRole, useSetUserSalaried, } from '@/hooks/useUserManagement';
...
function UsersTable({ users }) {
    ...
    const { data: staffRoles = [] } = useAllStaffRoles();
    const ROLES = staffRoles.filter(r => r.is_active).map(r => ({ value: r.name.toLowerCase(), label: r.name, description: r.description || '' }));
    const [sendingAccessEmail, setSendingAccessEmail] = useState(null);
    const [confirmCopyUser, setConfirmCopyUser] = useState(null);
    const handleSendAccessEmail = async (user) => {
        if (!user.email) {
        ...
        try {
            ...
            if (error)
                throw error;
            if (!data?.success)
                throw new Error(data?.error || 'Failed to send access email');
    ...
    const promptCopyInviteLink = (user) => {
      ...
      <TableBody>
        {users.length === 0 ? (<TableRow>
            <TableCell colSpan={7} className="text-center text-muted-foreground py-8">
              ...
              <TableCell>
                <Select value={user.role || ''} onValueChange={(newRole) => setUserRole.mutate({ userId: user.id, role: newRole })} disabled={setUserRole.isPending}>
                  <SelectTrigger className="w-[130px]">
                    <SelectValue placeholder="No role">
                      {user.role && (<Badge variant={getRoleBadgeVariant(user.role)}>
                          {user.role}
                        </Badge>)}
                    </SelectValue>
                  </SelectTrigger>
              ...
              <TableCell>
                <div className="flex items-center gap-2">
                  {user.registration_status === 'pending' ? (<Badge variant="secondary" className="gap-1">
                      <Clock className="h-3 w-3"/>
                      Pending
                    </Badge>) : user.registration_status === 'inactive' ? (<Badge variant="outline" className="gap-1 text-muted-foreground">
                      <XCircle className="h-3 w-3"/>
                      Inactive
                    </Badge>) : (<Badge variant="outline" className="gap-1 border-green-500/30 text-green-500">
              ...
              <TableCell className="text-center">
                <Switch checked={!!user.is_salaried} onCheckedChange={(checked) => setUserSalaried.mutate({ userId: user.id, isSalaried: checked })} disabled={setUserSalaried.isPending}/>
              </TableCell>
              <TableCell>
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end">
                    {user.registration_status === 'pending' && (<DropdownMenuItem onClick={() => handleSendAccessEmail(user)} disabled={sendingAccessEmail === user.id}>
                        <RefreshCw className="h-4 w-4 mr-2"/>
                        {sendingAccessEmail === user.id ? 'Resending...' : 'Resend Invite'}
                      </DropdownMenuItem>)}
                    <DropdownMenuItem onClick={() => handleSendAccessEmail(user)} disabled={sendingAccessEmail === user.id}>
                      <Mail className="h-4 w-4 mr-2"/>
                      {sendingAccessEmail === user.id ? 'Sending...' : 'Send Access Email'}
                    </DropdownMenuItem>
                    <DropdownMenuItem onClick={() => promptCopyInviteLink(user)} disabled={sendingAccessEmail === user.id}>
                      <RefreshCw className="h-4 w-4 mr-2"/>
                      Copy Invite Link
                    </DropdownMenuItem>
                    <DropdownMenuItem onClick={() => setUserActive.mutate({ userId: user.id, isActive: !user.is_active })}>
...
function InvitationsTable({ invitations }) {
    ...
    const canResend = (status) => status === 'pending' || status === 'provisioned' || status === 'failed' || status === 'emailed';
    const canRevoke = (status) => status === 'pending' || status === 'emailed';
    return (<Table>
      <TableHeader>
        <TableRow>
          <TableHead>Email</TableHead>
      ...
      <TableBody>
        {invitations.length === 0 ? (<TableRow>
            <TableCell colSpan={6} className="text-center text-muted-foreground py-8">
              No invitations
            </TableCell>
          </TableRow>) : (invitations.map((invite) => (<TableRow key={invite.id}>
              ...
              <TableCell>
                <div className="flex flex-col gap-1">
                  ...
                  {invite.error && (<span className="text-xs text-destructive">{invite.error}</span>)}
                </div>
              </TableCell>
              <TableCell className="text-muted-foreground">
              ...
              <TableCell>
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end">
                    {canResend(invite.status) && (<DropdownMenuItem onClick={() => resendInvitation.mutate(invite.id)} disabled={resendInvitation.isPending}>
                        <RefreshCw className="h-4 w-4 mr-2"/>
                    ...
                    {canRevoke(invite.status) && (<DropdownMenuItem onClick={() => revokeInvitation.mutate(invite.id)} disabled={revokeInvitation.isPending} className="text-destructive">
...
export default function UserManagement() {
    ...
    const pendingInvites = invitations.filter(i => i.status === 'pending' || i.status === 'emailed' || i.status === 'provisioned');
    return (<AppLayout>
      <div className="space-y-6">
        <PageHeader title="User Management" description="Manage team members and their access levels" actions={<InviteUserDialog />}/>
        ...
        <Tabs defaultValue="users">
          ...
          <TabsContent value="invitations" className="mt-4">
            <Card>
              <CardContent className="pt-6">
                {invitationsLoading ? (<div className="flex items-center justify-center py-8">
                    <div className="animate-spin h-6 w-6 border-2 border-primary border-t-transparent rounded-full"/>
                  </div>) : (<InvitationsTable invitations={invitations}/>)}
              </CardContent>
            </Card>
          </TabsContent>
        </Tabs>
