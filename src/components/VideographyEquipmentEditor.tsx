/**
 * VideographyEquipmentEditor - Kit-based equipment form for videographers
 *
 * Mirrors PhotographyEquipmentEditor but with video-focused categories:
 * Camera, Lenses, Audio, Lighting, Stabilisation, Other.
 */

import { useState, useEffect } from 'react';
import { Plus, Trash2, Video, Aperture, Lightbulb, Mic, Move3d, Loader2, Save, Pencil } from 'lucide-react';
...
import type { EquipmentItem, PhotographyEquipmentV2, StoredEquipment } from './PhotographyEquipmentEditor';
import { migrateToV2 } from './PhotographyEquipmentEditor';

export type VideoKitCategory = 'camera' | 'lenses' | 'audio' | 'lighting' | 'stabilisation' | 'other';

export const VIDEO_CATEGORY_CONFIG = [
  { key: 'camera' as const, label: 'Camera Bodies', icon: Video, placeholder: 'e.g., Sony FX3, Canon C70' },
  { key: 'lenses' as const, label: 'Lenses', icon: Aperture, placeholder: 'e.g., Sony 24-70 GM II' },
  { key: 'audio' as const, label: 'Audio', icon: Mic, placeholder: 'e.g., RØDE Wireless Pro, Sennheiser MKH 416' },
  { key: 'lighting' as const, label: 'Lighting', icon: Lightbulb, placeholder: 'e.g., Aputure 600D, LED Panel' },
  { key: 'stabilisation' as const, label: 'Stabilisation & Support', icon: Move3d, placeholder: 'e.g., DJI RS3 Pro, Tripod' },
  { key: 'other' as const, label: 'Other', icon: Aperture, placeholder: 'e.g., Cards, Batteries, Monitor' },
] as const;
...
interface VideographyEquipmentEditorProps {
...
export function VideographyEquipmentEditor({
  initialData,
  onChange,
  onSave,
  isSaving = false,
  readOnly = false,
}: VideographyEquipmentEditorProps) {
  ...
  const kitsForCategory = (cat: VideoKitCategory) =>
    data.kits.filter(k => (k.category as unknown as VideoKitCategory) === cat);

  const addKit = (cat: VideoKitCategory) => {
    const existing = kitsForCategory(cat);
    const cfg = VIDEO_CATEGORY_CONFIG.find(c => c.key === cat)!;
    const name = existing.length === 0 ? cfg.label : `${cfg.label} ${existing.length + 1}`;
    update({
      ...data,
      kits: [...data.kits, { id: generateId(), name, category: cat as any, items: [] }],
    });
  };
  ...
  const openRenameKit = (kit: { id: string; name: string }) => {
    ...
    <>
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>
                {readOnly
                  ? 'Your registered video gear'
                  : 'Add details about your camera bodies, lenses, audio, lighting and support gear'}
              </CardDescription>
            </div>
            {totalItems > 0 && (
        ...
        <CardContent className="space-y-4">
          <Accordion type="multiple" defaultValue={['camera', 'lenses']} className="space-y-2">
            {VIDEO_CATEGORY_CONFIG.map(({ key, label, icon: Icon, placeholder }) => {
                ...
                <AccordionItem key={key} value={key} className="border rounded-lg px-4">
                  <AccordionTrigger className="hover:no-underline">
                    <div className="flex items-center gap-2">
                      ...
                      <span>{label}</span>
                      {totalCatItems > 0 && (
                        <span className="text-xs text-muted-foreground ml-1">({totalCatItems})</span>
                      )}
                      {kits.length > 1 && (
                        <span className="text-xs text-muted-foreground">· {kits.length} kits</span>
                      )}
                    </div>
                  </AccordionTrigger>
